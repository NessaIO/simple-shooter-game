...
ship = Player(img_hero, 5, win_height - 105, 80, 100, 10)
...
while game:
    for e in event.get():
        if e.type == QUIT:
            ...
            if e.key == K_SPACE:
                if num_fire < 5 and rel_time == False and shoots != 0:
                    num_fire += 1
                    fire_sound.play()
                    ship.fire()
                    shoots -= 1
    ...
    if finish != True:
        ...
        text_shoots = font2.render("Выстрелы: " + str(shoots), 1, (255, 255, 255))
        window.blit(text_shoots, (10, 80))
        ...
        collides_1 = sprite.groupcollide(asteroids, bullets, True, True)
